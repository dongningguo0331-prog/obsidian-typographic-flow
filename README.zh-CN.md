...
#### 禅模式（Zen Mode）
将**所有非活动行**调暗到可配置的透明度。三种模式可选：

| 模式 | 行为 | 适用场景 |
|------|------|---------|
| **固定（Static）** | 由滑块设定的固定透明度 | 一致的微弱虚化 |
| **动态响应（Dynamic）** | 透明度根据打字速度实时变化 | 高速输入时深度聚焦，停笔时温和回看 |
| **关闭（Off）** | 禅模式禁用 | 普通编辑 |
...
| Toggle Zen Mode | 循环切换禅模式（关 → 固定 → 动态 → 关） | 关 |
...
| 设置 | 类型 | 范围 | 默认值 | 说明 |
|------|------|------|--------|------|
| Zen Mode / 禅模式 | 下拉 | off/static/dynamic | off | 固定：固定透明度。动态：根据打字速度自适应 |
| Zen Opacity / 禅模式透明度 | 滑块 | 0–100% | 25% | 调暗行的亮度（仅固定模式） |
...
| Auto Calibrate / 自动校准 | 开关 | — | 关 | 根据主题字体自动计算最佳网格模数 |
